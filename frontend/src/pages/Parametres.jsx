...
import {
  ...
  FileJson, FileText, AlertTriangle
} from 'lucide-react';
...
function OngletBoutique() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="font-syne text-lg font-bold text-gray-900 mb-1">Informations de la boutique</h2>
        <p className="text-sm text-gray-400">Ces informations apparaîtront sur vos reçus de vente</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-gray-500">Pays</label>
          <input className={inputCls} value={form.pays} onChange={e => set('pays', e.target.value)} placeholder="Guinée"/>
        </div>
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-gray-500">Devise</label>
      ...
      <div className="flex justify-end">
        <button onClick={sauvegarder}
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md ${saved ? 'bg-green-500 text-white shadow-green-200' : 'gradient-brand text-white shadow-blue-200 hover:opacity-90'}`}>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-syne text-lg font-bold text-gray-900 mb-1">Catégories de produits</h2>
          <p className="text-sm text-gray-400">{cats?.length || 0} catégorie(s) configurée(s)</p>
        </div>
        <button onClick={() => ouvrirModale()}
          className="flex items-center gap-2 px-4 py-2.5 gradient-brand text-white font-semibold text-sm rounded-xl hover:opacity-90 transition-all shadow-md shadow-blue-200">
          <Plus size={15}/> Nouvelle catégorie
        </button>
      </div>

      {isLoading ? (
        <div className="flex justify-center py-12"><div className="w-7 h-7 border-2 border-gray-200 border-t-blue-500 rounded-full animate-spin"/></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {cats?.map(c => (
            <div key={c._id} className="flex items-center gap-3 p-4 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{c.nombreProduits || 0} produit(s)</p>
              </div>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => ouvrirModale(c)}
                  className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-all"><Pencil size={13}/></button>
                <button onClick={() => window.confirm(`Supprimer "${c.nom}" ?`) && deleteMutation.mutate(c._id)}
                  className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all"><Trash2 size={13}/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modaleF !== null && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in"
          onClick={e => e.target === e.currentTarget && setModaleF(null)}>
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-scale-in">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-syne font-bold text-gray-900">{modaleF._id ? 'Modifier' : 'Nouvelle'} catégorie</h3>
              <button onClick={() => setModaleF(null)} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100"><X size={15}/></button>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {COULEURS.map(c => (
                    <button key={c} onClick={() => setForm(f=>({...f,couleur:c}))}
                      className={`w-8 h-8 rounded-xl transition-all hover:scale-110 ${form.couleur === c ? 'ring-2 ring-offset-2 ring-gray-400 scale-110' : ''}`}
                      style={{ background: c }}/>
                  ))}
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <div className="w-6 h-6 rounded-lg" style={{ background: form.couleur }}/>
                  <span className="text-xs font-mono text-gray-500">{form.couleur}</span>
                </div>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50 rounded-b-2xl">
              <button onClick={() => setModaleF(null)} className="px-4 py-2 rounded-xl text-sm text-gray-500 hover:bg-gray-100 transition-all">Annuler</button>
              <button onClick={() => mutation.mutate(form)} disabled={!form.nom || mutation.isPending}
...
function OngletPreferences() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="font-syne text-lg font-bold text-gray-900 mb-1">Préférences</h2>
        <p className="text-sm text-gray-400">Configurez le comportement de l'application</p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm font-semibold text-gray-800">Seuil d'alerte par défaut</p>
              <p className="text-xs text-gray-400 mt-0.5">Quantité minimum avant déclenchement d'une alerte</p>
            </div>
            <input type="number" min="0" className="w-20 bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-xl px-3 py-2 outline-none focus:border-blue-400 text-center"
        ...
        <div className="p-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex flex-col gap-2">
            <div>
              <p className="text-sm font-semibold text-gray-800">Email pour les alertes</p>
              <p className="text-xs text-gray-400 mt-0.5">Recevez les alertes de stock par email (bientôt disponible)</p>
            </div>
        ...
        <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-3">
          ...
          <div>
            ...
            <button className="text-xs font-semibold text-red-600 bg-red-50 border border-red-200 px-3 py-1.5 rounded-lg hover:bg-red-100 transition-all">
      ...
      <div className="flex justify-end">
        <button onClick={sauvegarder}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold gradient-brand text-white hover:opacity-90 transition-all shadow-md shadow-blue-200">
...
function OngletExport() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="font-syne text-lg font-bold text-gray-900 mb-1">Export des données</h2>
        <p className="text-sm text-gray-400">Téléchargez vos données en CSV ou JSON</p>
      </div>
          ...
          <div key={e.endpoint} className={`flex items-center justify-between p-4 rounded-2xl border ${colorMap[e.color]}`}>
            ...
            <div className="flex gap-2">
              <button onClick={() => exporter(e.endpoint, 'csv')} disabled={loading[`${e.endpoint}-csv`]}
                className="flex items-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-all disabled:opacity-50">
                <FileText size={12}/> CSV
              </button>
              <button onClick={() => exporter(e.endpoint, 'json')} disabled={loading[`${e.endpoint}-json`]}
                className="flex items-center gap-1.5 px-3 py-2 gradient-brand text-white rounded-xl text-xs font-semibold hover:opacity-90 transition-all disabled:opacity-50 shadow-sm">
      ...
      <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 text-xs text-gray-400">
        💡 <strong>CSV</strong> s'ouvre dans Excel · <strong>JSON</strong> pour les développeurs ou la sauvegarde
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-5 lg:p-7">
      <div className="mb-6 animate-fade-up">
        <h1 className="font-syne text-xl font-bold text-gray-900 tracking-tight">Paramètres</h1>
        <p className="text-sm text-gray-400 mt-1">Configurez votre application et votre boutique</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 animate-fade-up-2">
          ...
          <div className="card-neu p-2">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setOnglet(t.id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${onglet === t.id ? 'gradient-brand text-white shadow-md shadow-blue-200' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}>
                <t.Icon size={15}/>
                {t.label}
              </button>
            ))}
        ...
        {/* Contenu */}
        <div className="flex-1 card-neu p-6 animate-fade-up-3">
          {TabContent[onglet]}
        </div>
      </div>
    </div>
