    ...
    <div className="min-h-screen bg-slate-100 p-5 lg:p-7">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 animate-fade-up">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-syne text-2xl font-bold text-slate-900 tracking-tight">Statistiques</h1>
            <p className="text-sm text-slate-400 mt-0.5">Analyse de vos performances commerciales</p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={WifiOff} title="Impossible de charger les statistiques" description="Une erreur est survenue lors du chargement. Vérifiez votre connexion et réessayez." />
      ) : (
        <>
          {/* KPIs */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5 animate-fade-up-2">
            {[
              { label:'CA ce mois',    value:fmtM(kpis.caMois)+' GNF',  Icon:TrendingUp,  color:'text-indigo-600', bg:'bg-indigo-50', border:'border-l-indigo-500', trend:tendance },
              { label:'Ventes ce mois',value:kpis.nbVentesMois||0,      Icon:ShoppingCart,color:'text-indigo-600', bg:'bg-indigo-50', border:'border-l-indigo-500' },
              { label:'Panier moyen',  value:fmtM(kpis.panierMoyen)+' GNF', Icon:BarChart2, color:'text-green-600', bg:'bg-green-50', border:'border-l-green-500' },
              { label:'Remises totales',value:fmtM(kpis.remisesTotales)+' GNF', Icon:Calendar, color:'text-amber-600', bg:'bg-amber-50', border:'border-l-amber-500' },
            ].map((k, i) => (
              <div key={k.label} className={`card-neu border-l-4 ${k.border} p-4 hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 animate-fade-up-${i+1}`}>
                ...
                {k.trend !== undefined && (
                  <div className={`flex items-center gap-1 mt-1.5 text-[10px] font-bold ${k.trend >= 0 ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <div className="card-neu p-5 mb-4 animate-fade-up-3">
            <div className="flex items-center justify-between mb-5">
              <div>
                <p className="text-sm font-bold text-slate-800">Chiffre d'affaires par jour</p>
                <p className="text-xs text-slate-400 mt-0.5">Évolution sur {periode} jours</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-1.5 rounded-full" style={{ background:'#4F46E5' }}/>
                  <span className="text-xs text-slate-400">CA (GNF)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-1.5 rounded-full" style={{ background:'#A5B4FC' }}/>
                  <span className="text-xs text-slate-400">Nb ventes</span>
                </div>
              </div>
            </div>
            <ResponsiveContainer width="100%" height={240}>
              <AreaChart data={data?.ventesParJour || []} margin={{ top:5, right:5, bottom:0, left:-10 }}>
                ...
                <Area type="monotone" dataKey="ca" name="CA" stroke="#4F46E5" strokeWidth={2.5}
                  fill="url(#gCA)" dot={false} activeDot={{ r:5, fill:'#4F46E5', stroke:'#fff', strokeWidth:2 }}/>
                <Area type="monotone" dataKey="nb" name="Nb ventes" stroke="#A5B4FC" strokeWidth={2}
                  fill="url(#gNb)" dot={false} activeDot={{ r:4, fill:'#A5B4FC', stroke:'#fff', strokeWidth:2 }}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4 animate-fade-up-4">
            ...
            <div className="card-neu p-5">
              <p className="flex items-center gap-1.5 text-sm font-bold text-slate-800 mb-4">
                <Trophy size={14} className="text-amber-500"/> Top 10 produits
              </p>
            ...
            {/* Modes de paiement */}
            <div className="card-neu p-5">
              <p className="flex items-center gap-1.5 text-sm font-bold text-slate-800 mb-4"><CreditCard size={14} className="text-indigo-500"/> Modes de paiement</p>
                    ...
                    {data.parModePaiement.map((m, i) => {
                      ...
                      return (
                        <div key={m._id} className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-slate-600 flex-1">{modeLabels[m._id] || m._id}</span>
                          <span className="text-xs font-bold text-slate-800">{fmt(m.ca)} GNF</span>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 animate-fade-up-5">
            ...
            {/* Top clients */}
            <div className="card-neu p-5">
              <p className="flex items-center gap-1.5 text-sm font-bold text-slate-800 mb-4"><Users size={14} className="text-indigo-500"/> Top 10 clients</p>
              {data?.topClients?.length > 0 ? (
                <div className="flex flex-col gap-2">
                  {data.topClients.map((c, i) => (
                    <div key={c._id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-slate-800 truncate">{c._id || 'Client comptoir'}</p>
                        <p className="text-[10px] text-slate-400">{c.nbAchats} achat(s) · panier moy. {fmtM(c.panier)} GNF</p>
                      </div>
                      <span className="text-xs font-bold text-indigo-600 flex-shrink-0">{fmtM(c.ca)} GNF</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            {/* Heures de pointe */}
            <div className="card-neu p-5">
              <p className="flex items-center gap-1.5 text-sm font-bold text-slate-800 mb-4"><Clock3 size={14} className="text-indigo-500"/> Heures de pointe</p>
